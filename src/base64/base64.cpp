...
std::string base64_encode(const char* buf, size_t bufLen) {
    ...
    while (bufLen--) {
        ...
        if (i == 3) {
            ...
            char_array_4[1] = static_cast<char>(((char_array_3[0] & 0x03) << 4) + ((char_array_3[1] & 0xf0) >> 4));
            char_array_4[2] = static_cast<char>(((char_array_3[1] & 0x0f) << 2) + ((char_array_3[2] & 0xc0) >> 6));
